...
class Environment(BaseModel):
    ...
    key = models.CharField(_("Key"), max_length=100, help_text="Variable name or key")
    value = models.TextField(
        _("Value"), help_text="Variable value (may be encrypted for secret type)"
    )
    ...
    description = models.TextField(
        _("Description"), blank=True, help_text="Optional description of this variable"
    )
    is_enabled = models.BooleanField(
        _("Enabled"), default=True, help_text="Whether this variable is enabled"
    )
    ...
    file_content = models.BinaryField(
        blank=True, null=True, help_text="Binary content of uploaded file"
    )
    ...
    file_type = models.CharField(
        max_length=100, blank=True, null=True, help_text="MIME type of uploaded file"
    )
    ...
    def clean(self):
        ...
        if not re.match(r"^[a-zA-Z0-9_]+$", self.key):
            raise ValidationError(
                {"key": _("Key must contain only letters, numbers, and underscores.")}
            )

        # Check for duplicate keys in the same environment
        duplicates = Environment.objects.filter(
            project=self.project, key__iexact=self.key
        )
        ...
        if duplicates.exists():
            raise ValidationError(
                {"key": _("This environment already has a variable with this key.")}
            )

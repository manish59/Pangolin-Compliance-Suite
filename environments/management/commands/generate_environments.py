...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--project-id', type=str, help='Specific project ID to generate environments for')
    ...
    def handle(self, *args, **options):
        ...
        if project_id:
            try:
                ...
                self.stdout.write(f"Generating environments for specific project: {projects[0].name}")
        ...
        for project in projects:
            self.stdout.write(f"Generating {count} environment variables for project: {project.name}")
            ...
            for i in range(count):
                ...
                if (i + 1) % 10 == 0:
                    self.stdout.write(f"  Created {i + 1} environment variables for {project.name}")

            self.stdout.write(
                self.style.SUCCESS(f"Successfully created {count} environment variables for project {project.name}"))

        self.stdout.write(self.style.SUCCESS(f"Total environment variables created: {total_created}"))
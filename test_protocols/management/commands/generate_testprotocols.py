...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=5,
            help="Number of protocols to create per test suite",
        )
        parser.add_argument(
            "--suite-id",
            type=str,
            help="Specific test suite ID to generate protocols for",
        )
        parser.add_argument(
            "--steps",
            type=int,
            default=3,
            help="Number of execution steps per protocol",
        )
        parser.add_argument(
            "--verifications",
            type=int,
            default=2,
            help="Number of verification methods per step",
        )
    ...
    def handle(self, *args, **options):
        ...
        if suite_id:
            try:
                ...
                self.stdout.write(
                    f"Generating protocols for specific test suite: {suites[0].name}"
                )
            except TestSuite.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(f"Test suite with ID {suite_id} not found")
                )
                return
        else:
            ...
            if not suites.exists():
                self.stdout.write(
                    self.style.ERROR(
                        "No test suites found. Please create a test suite first."
                    )
                )
                return
        ...
        for suite in suites:
            self.stdout.write(
                f"Generating {count} protocols for test suite: {suite.name}"
            )

            # Get existing protocol names to avoid duplicates
            existing_protocol_names = set(
                TestProtocol.objects.filter(suite=suite).values_list("name", flat=True)
            )
            ...
            for i in range(count):
                ...
                description = (
                    f"This is a test protocol for {suite.name} ({protocol_type})"
                )
                ...
                step_verifications = self._create_execution_steps(
                    protocol, steps_count, verifications_count
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created {count} protocols for test suite {suite.name}"
                )
            )
        ...
        if config_type == "database":
            ...
            config_data = {
                "auth_method": random.choice(
                    ["access_key", "profile", "instance_role"]
                ),
    ...
    def _create_execution_steps(self, protocol, steps_count, verifications_count):
        ...
        for i in range(steps_count):
            ...
            if step_type == "Prepare":
                ...
                kwargs = {
                    "command": random.choice(
                        ["get_data", "process_items", "trigger_action"]
                    ),
                    "parameters": {
            ...
            step_verifications = self._create_verification_methods(
                step, verifications_count
            )
            total_verifications += step_verifications
    ...
    def _create_verification_methods(self, step, count):
        ...
        for i in range(count):
            ...
            supports_comparison = any(
                t in verification_type
                for t in ["string", "numeric", "api_status", "db_row"]
            )

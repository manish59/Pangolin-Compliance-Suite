...
class TestSuite(BaseModel):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="test_suite"
    )
...
class TestProtocol(BaseModel):
    ...
    suite = models.ForeignKey(
        TestSuite, on_delete=models.CASCADE, related_name="protocols"
    )
    name = models.CharField(max_length=100)
    description = models.TextField(blank=True, null=True)
...
class ConnectionConfig(BaseModel):
    ...
    protocol = models.OneToOneField(
        TestProtocol, on_delete=models.CASCADE, related_name="connection_config"
    )
...
class ProtocolRun(BaseModel):
    ...
    protocol = models.ForeignKey(
        TestProtocol, on_delete=models.CASCADE, related_name="runs"
    )
    ...
    result_status = models.CharField(
        max_length=20, choices=RESULT_STATUS_CHOICES, blank=True, null=True
    )
...
class ExecutionStep(BaseModel):
    ...
    test_protocol = models.ForeignKey(
        TestProtocol, on_delete=models.CASCADE, related_name="steps"
    )
    # Keyword arguments (kwargs) stored as JSON object
    kwargs = models.JSONField(
        default=dict, help_text=_("Keyword arguments (kwargs) as a JSON object")
    )
...
class VerificationMethod(BaseModel):
    ...
    comparison_method = models.CharField(
        max_length=50, choices=COMPARISON_OPERATOR_CHOICES
    )
    expected_result = models.JSONField(default=dict)
    # Additional configuration for this verification method
    config_schema = models.JSONField(
        default=dict, help_text=_("JSON schema for configuration parameters")
    )

    # Requirements for this verification method
    requires_expected_value = models.BooleanField(
        default=True, help_text=_("Whether this method requires an expected value")
    )
    supports_dynamic_expected = models.BooleanField(
        default=False,
        help_text=_(
            "Whether this method supports dynamic expected values from environment variables"
        ),
    )
    ...
    def verify(self, actual_value, expected_result=None, config_schema=None):
        ...
        try:
            result = verifier.verify(
                ...
                comparison_method=(
                    self.comparison_method if self.supports_comparison else None
                ),
...
class VerificationResult(BaseModel):
    ...
    verification_time = models.DateTimeField(
        auto_now_add=True, help_text="When this verification was performed"
    )

    # Verification outcome
    success = models.BooleanField(
        default=False, help_text="Whether the verification passed or failed"
    )
    ...
    actual_value = models.JSONField(
        null=True, help_text="The actual value that was verified"
    )
    expected_value = models.JSONField(
        null=True, help_text="The expected value used for verification"
    )
    ...
    error_stack_trace = models.TextField(
        blank=True, null=True, help_text="Stack trace from verification error"
    )

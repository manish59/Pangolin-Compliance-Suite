...
class TestProtocolCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        if TestProtocol.objects.filter(suite=suite, name=name).exists():
            form.add_error(
                "name", "A protocol with this name already exists in this suite."
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if selected_connection_id:
            try:
                ...
                existing_connection = ConnectionConfig.objects.get(
                    pk=selected_connection_id
                )
            ...
            try:
                ...
                if isinstance(config_data, str):
                    ...
                    yaml_string = yaml.dump(
                        value, default_flow_style=False, indent=2, sort_keys=False
                    )
...
class ProtocolRunDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        verification_results_dict = {
            vr.verification_step_id: vr for vr in verification_results
        }
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            if protocols:
                ...
                messages.warning(
                    request, "No active protocols found in the test suite."
                )

        except Exception as e:
            # Add an error message
            messages.error(request, f"Failed to run test suite: {str(e)}")

        # Redirect back to the test suite detail page
        return HttpResponseRedirect(
            reverse("testsuite:testsuite_detail", kwargs={"pk": pk})
        )
...
class ExecutionStepCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["test_protocol"] = (
            self.protocol
        )  # Note: make sure this variable matches your template
        context["protocol"] = self.protocol  # Keep both to ensure compatibility
        return context

    def form_valid(self, form):
        form.instance.test_protocol = get_object_or_404(
            TestProtocol, pk=self.kwargs["protocol_id"]
        )
    ...
    def get_success_url(self):
        return reverse(
            "testsuite:step_list", kwargs={"protocol_id": self.kwargs["protocol_id"]}
        )
...
class ExecutionStepDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse(
            "testsuite:step_list", kwargs={"protocol_id": self.object.test_protocol.id}
        )
...
class ExecutionStepReorderView(LoginRequiredMixin, View):
    ...
    def post(self, request, protocol_id):
        ...
        active_runs = ProtocolRun.objects.filter(
            protocol=protocol, status__in=["started", "running"]
        ).exists()
            ...
            with transaction.atomic():
                ...
                for index, step_id in enumerate(step_order):
                    step = get_object_or_404(
                        ExecutionStep, pk=step_id, test_protocol=protocol
                    )
...
class ExecutionStepCloneView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if active_runs:
            ...
            return HttpResponseRedirect(
                reverse("testsuite:step_list", kwargs={"protocol_id": protocol.id})
            )
        ...
        return HttpResponseRedirect(
            reverse("testsuite:step_detail", kwargs={"pk": new_step.pk})
        )
...
class RunExecutionStepsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return ExecutionStep.objects.filter(test_protocol=self.run.protocol).order_by(
            "order_index"
        )
...
class ExecuteStepView(LoginRequiredMixin, View):
    ...
    def post(self, request, run_id, step_id):
        ...
        if run.status not in ["started", "running"]:
            messages.error(
                request, f"Cannot execute step: run is in {run.status} state"
            )
            return HttpResponseRedirect(
                reverse("testsuite:run_steps", kwargs={"run_id": run.id})
            )
...
class ProtocolVerificationListView(ListView):
    ...
    def get_queryset(self):
        ...
        self.protocol = get_object_or_404(TestProtocol, pk=self.kwargs["protocol_id"])
        return VerificationMethod.objects.filter(
            execution_step__test_protocol=self.protocol
        )
...
class ProtocolVerificationCreateView(CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["execution_step"].queryset = ExecutionStep.objects.filter(
            test_protocol=protocol
        )
        return form
    ...
    def get_success_url(self):
        return reverse(
            "testsuite:protocol_verification_detail",
            kwargs={"protocol_id": self.kwargs["protocol_id"], "pk": self.object.pk},
        )
...
class ProtocolVerificationDeleteView(DeleteView):
    ...
    def get_success_url(self):
        return reverse(
            "testsuite:protocol_verification_list",
            kwargs={"protocol_id": self.kwargs["protocol_id"]},
        )

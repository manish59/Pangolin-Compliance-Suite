...
class ConfigurationValidator:
    ...
    @staticmethod
    def validate(config: Any, validation_rules: Optional[Dict[str, Dict[str, Any]]] = None) -> List[str]:
        ...
        for field_name, rules in validation_rules.items():
            ...
            field_errors = ConfigurationValidator._validate_field(field_name, field_value, rules)
            errors.extend(field_errors)

        # Add any complex cross-field validations
        context_errors = ConfigurationValidator._validate_context(config, validation_rules)
    ...
    def _get_default_rules(config: Any) -> Dict[str, Dict[str, Any]]:
        ...
        if 'AWS' in config_type:
            return {
                'auth_method': {
                    'required': True,
                    'choices': ['ACCESS_KEY', 'PROFILE', 'INSTANCE_ROLE', 'WEB_IDENTITY', 'SSO']
                },
                'region': {
                    'required': True,
                },
        ...
        # Database connection config
            ...
            return {
                ...
                'service_name': {
                    'required_if_custom': {
                        'condition': lambda cfg: cfg.database_type == 'ORACLE' and
                                                 not cfg.connection_string and
                                                 not cfg.sid and not cfg.tns_name
                    }
                }
            }
        ...
        # Kubernetes connection config
            ...
            return {
                ...
                'kubeconfig_path': {
                    'required_if_custom': {
                        'condition': lambda cfg: cfg.auth_method == 'CONFIG' and not cfg.in_cluster
                    }
                },
                'in_cluster': {
                    'required_if_custom': {
                        'condition': lambda cfg: cfg.auth_method == 'CONFIG' and not cfg.kubeconfig_path
                    }
                },
                'api_token': {
                ...
                'username': {
                    'required_if': {'field': 'auth_method', 'value': 'BASIC'}
                },
                'password': {
                    'required_if': {'field': 'auth_method', 'value': 'BASIC'}
                }
        ...
        # SSH connection config
            ...
            return {
                ...
                'key_filename': {
                    'required_if_custom': {
                        'condition': lambda cfg: cfg.auth_method == 'PUBLIC_KEY' and
                                                 not cfg.encrypted_key_str
                    }
                },
                'encrypted_key_str': {
                    'required_if_custom': {
                ...
                'passphrase': {
                    'required_if_custom': {
                        'condition': lambda cfg: cfg.auth_method == 'PUBLIC_KEY' and
                                                 cfg.encrypted_key_str and not cfg.key_filename
                    }
                }
            }
        ...
        # API connection config
            ...
            return {
                'auth_method': {
                    'required': True,
                    'choices': ['NONE', 'BASIC', 'BEARER', 'JWT', 'API_KEY', 'OAUTH2', 'HMAC']
                },
                'username': {
                    'required_if': {'field': 'auth_method', 'value': 'BASIC'}
                },
        ...
        # Azure connection config
            ...
            return {
                ...
                'client_id': {
                    'required_if': {'field': 'auth_method', 'value': 'SERVICE_PRINCIPAL'}
                },
                'client_secret': {
                    'required_if': {'field': 'auth_method', 'value': 'SERVICE_PRINCIPAL'}
                },
    ...
    @staticmethod
    def _validate_field(field_name: str, field_value: Any, rules: Dict[str, Any]) -> List[str]:
        ...
        if 'choices' in rules and field_value is not None:
            ...
            if field_value not in choices and str(field_value) not in choices:
                errors.append(f"'{field_name}' must be one of: {', '.join(str(c) for c in choices)}")
        ...
        if 'pattern' in rules and field_value is not None and isinstance(field_value, str):
            import re
    ...
    def _validate_context(config: Any, rules: Dict[str, Dict[str, Any]]) -> List[str]:
        ...
        for field_name, field_rules in rules.items():
            ...
            if 'required_if' in field_rules:
                ...
                conditions = required_if if isinstance(required_if, list) else [required_if]
                ...
                for condition in conditions:
                    ...
                    if other_value == expected_value or str(other_value) == expected_value:
                        if not field_value:
                            errors.append(
            ...
            if 'required_unless' in field_rules:
                ...
                # Handle both single condition and list of conditions
                conditions = required_unless if isinstance(required_unless, list) else [required_unless]
                ...
                if is_required and not field_value:
                    ...
                    errors.append(f"'{field_name}' is required unless {fields_list} meets conditions")
            ...
            if 'required_if_custom' in field_rules:
                ...
                if condition_func(config) and not field_value:
                    errors.append(f"'{field_name}' is required based on other configuration settings")

...
def oauth2_callback(request):
    ...
    if 'error' in token_data:
        messages.error(request, f"OAuth error: {token_data.get('error_description', token_data['error'])}")
        return redirect('authentication:login')
...
def openid_callback(request):
    ...
    if 'error' in token_data:
        messages.error(request, f"OpenID error: {token_data.get('error_description', token_data['error'])}")
        return redirect('authentication:login')

...
class DashboardView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if date_str:
            try:
                ...
                queryset = queryset.filter(started_at__gte=start_date, started_at__lt=end_date)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['running_protocols'] = ProtocolRun.objects.filter(status='running').count()
        ...
        recent_runs = ProtocolRun.objects.select_related('protocol').order_by('-started_at')[:10]
        ...
        recent_protocols = TestProtocol.objects.select_related('suite').order_by('-created_at')[:5]
